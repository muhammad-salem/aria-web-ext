...
chrome.downloads.onDeterminingFilename.addListener(
	function(downloadItem, suggest) {
		...
		*/
		...
		UrlToAria(downloadItem.id, downloadItem.url, 
                  downloadItem.referrer, downloadItem.filename, 
                  downloadItem.fileSize, downloadItem.mime);
	...
	chrome.contextMenus.onClicked.addListener(
		function(info, tab) {
			switch(info.menuItemId)
			{
				...
				case "page":
					UrlToAria(-1, tab.url, tab.url, true, -1, '');
					break;
	...
	function(details) {
		...
		if(details.method == "POST")
		{
			...
			if(file && _config.monitor)
			{
				...
				UrlToAria(-1, details.url, referrer, '', false,
					request_bodies[details.requestId], -1, '');
